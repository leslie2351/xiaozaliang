            ...
            <Card style="width:99.5%; margin:0 auto;">
                ...
                <Card class="grouping_manage">
                    ...
                    <Collapse accordion v-model="value22">
                        <Panel v-for='(item,index) in dataList'>
                            <div class="head_main">
                                    <span class="head_main_h">{{item.groupName}}</span>
                                    <span class="head_main_b">{{item.sort}}</span>
                                    ...
                                    <span class="head_main_b">
                                        ...
                                        <Button type="error" @click.stop='btnDel(item)'>删除</Button>
                                    </span>
                            </div>
                            <div slot="content" v-if='item.storeGroupInfoPOs'>
                                <div class="headSlot clearfix" v-for='child,n in item.storeGroupInfoPOs'>
                                    ...
                                    <span class="head_main_b">{{child.sort}}</span>
                                    ...
                                    <span class="head_main_b">
                                        ...
                                        <Button type="error" @click.stop='btnDel(child)'>删除</Button>
                                    </span>
                                </div>
    ...
    methods: {
        ...
        btn(id,index,n){ //点击修改
            ...
            if(n == undefined){
                ...
                this.value2 = this.dataList[index].sort;
                this.sortOld = this.dataList[index].sort;
                ...
                this.value2 = this.dataList[index].storeGroupInfoPOs[n].sort;
                this.sortOld = this.dataList[index].storeGroupInfoPOs[n].sort;
        ...
        btnDel(val){ //点击删除的时候
            ...
            this.delModal = true;
            this.id = val.id;
            this.parentGroupId = val.parentGroupId;
            this.sortValue = [];
            this.sortValue.push(val.sort)
        },
        okDel(){ //确认删除
            let ids = "'" +this.id+"'"
        ...
        //点击添加时获取最大值
        getMaxNum(){
            this.$http.post('http://192.168.5.11:8080/storeGroup/maxSort',{
                storeId:6666,
                parentGroupId:this.parentGroupId,
            }).then((data)=>{
                console.log(data);
                this.maxNum = data.data;
            }).catch((err)=>{
                console.log(err);
