        ...
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
            ...
            <FormItem label="品牌：" prop="text1">
                <!-- <Input v-model="formValidate.text1" placeholder="品牌" style="width: 343px"></Input>
                 -->
                <Select class='fl margin-right-10' v-model="formValidate.text1" style="width:200px" placeholder='品牌' @on-change='screenOrder'>
                    <Option v-for="item in brand" :value="item.brandName" :key="item.brandName">{{item.brandName}}</Option>
                </Select>
            </FormItem>
...
import VueImgInputer from 'vue-img-inputer'
    export default {
        ...
        methods: {
            handleSubmit (name) {
                console.log(this.$store.state.app.classifyId);
                
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        // this.$Message.success('请继续!');
                        // this.$store.commit('setSetp');
                        this.$http.post('http://192.168.5.11:8080/storeGoods/addStoreGoods',{
                            storeId:6666,
                            goodsName:this.formValidate.text2,
                            classifyId:this.$store.state.app.classifyId,
                            brandId:this.brandId,
                            purchasePrice:this.formValidate.text3,
                            salePrice:this.formValidate.text4,
                            goodsWeight:this.formValidate.text5,
                            totalStock:this.formValidate.text6,
            ...
            screenOrder(val){ //选择品牌时获取品牌id
              
                this.brand.forEach((item)=>{
                    if(val == item.brandName){
        ...
        mounted(){
            this.$http.post('http://192.168.5.11:8080/goodsInfo/goodsBrand',{
