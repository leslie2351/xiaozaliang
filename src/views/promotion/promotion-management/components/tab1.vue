                        ...
                        <div>
                            <div class="clearfix">
                                ...
                                <div style="padding-top:20px;">
                                    ...
                                    <div class="padding-top-10 clearfix">
                                        <span class="fl promotion-t margin-right-10">优惠门槛：</span>
                                        <div style="width: 250px" class="fl">
                                            满：<Input v-model="value4" placeholder="..." style="width: 100px"></Input> 元
                    
                                        </div>
                                    </div>
                                    <div class="padding-top-10 clearfix preferential-way">
                                        <span class="fl promotion-t margin-right-10">优惠方式：</span>
                                        
                                        <div class="clearfix fl">
                                            <div style="height:32px;">
                                                <Checkbox v-model="jain" style="margin-top:8px;">
                                                    &nbsp 减
                                                </Checkbox>
                                                <span v-if="jain">：<Input v-model="value5" placeholder="..." style="width: 100px"></Input> 元</span>
                                            </div>
                                            <div class="padding-top-10">
                                                <Checkbox v-model="jain2">&nbsp 免邮费</Checkbox>
                                            </div>
                                            <div class="padding-top-10" style="height:45px;">
                                                <Checkbox v-model="jain3" style="margin-top:8px;" :disabled ='whether'>&nbsp 送赠品
                                                   
                                                </Checkbox>
                                                <Alert type="warning" show-icon class="fr margin-left-10" v-if='whether == true'>当前无赠品</Alert>
                                               <div v-if="jain3" class="fr " style="position: relative;">
                                                    <Select class='margin-right-10' placement='top' v-model="value6" style="width:150px;        "    
                                                        @on-change='screenOrder'
                                                    >
                                                        <Option v-for="item in dictType" :value="item.gift_name" :key="item.gift_name">
                                                            {{item.gift_name}}
                                                        </Option>
                                                    </Select>
                                               </div>
...
export default {
    ...
    methods:{
        ...
        screenOrder(val){ //点击赠品获取当前赠品的id
            this.dictType.forEach((item)=>{
                if(val == item.gift_name){
        ...
        //获取赠品下拉框
        getScreenOrder(){
            this.$http.post('http://192.168.5.21:8080/store-gift-info/list',{
                store_id:6666
            }).then((data)=>{
                ...
                if(data.data.length == 0){
                    ...
                    this.value6 = this.dictType[0].gift_name;
                    this.gift_id = this.dictType[0].id;
                }
        ...
        okAdd(){ //确认添加
            ...
            let is_free_shipping = this.jain2 == true ? 1 : 0 ;
            let is_give_gift = this.jain3 == true ? 1 : 0 ;
            ...
            if(this.status == '新增'){
                this.$http.post('http://192.168.5.21:8080/store-promotion-info/add',{
                    ...
                    full_money:this.value4,
                    reduction_money:this.value5,
        ...
        btnAttr(index){
            ...
            this.value4 = this.orderData[index].full_money;
            this.value5 = this.orderData[index].reduction_money;
            if(this.value5.length != ''){
                this.jain = true;
            }
            if(this.orderData[index].is_free_shipping == 1){
                this.jain2 = true;
            ...
            if(this.orderData[index].is_give_gift == 1){
                this.jain3 = true;
                this.getPromotionGift()

                this.dictType.forEach((item)=>{
                    if(this.gift_id == item.id){
                        this.value6 = item.gift_name
        ...
        //修改时获取当前活动的赠品信息
        getPromotionGift(){   
            this.$http.get('http://192.168.5.21:8080/store-promotion-info/getPromotionGift/'+this.id)
            .then((data)=>{
    ...
    mounted(){
        this.initTable()
        this.getScreenOrder()//获取新增时下拉框内容
        this.initSeero()
